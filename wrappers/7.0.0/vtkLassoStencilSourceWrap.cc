...
#ifdef VTK_NODE_PLUS_VTKLASSOSTENCILSOURCEWRAP_INITPTPL
	VTK_NODE_PLUS_VTKLASSOSTENCILSOURCEWRAP_INITPTPL
#endif
	ptpl.Reset( tpl );
}
...
void VtkLassoStencilSourceWrap::New(const Nan::FunctionCallbackInfo<v8::Value>& info)
{
	...
	if(info.Length() == 0)
	{
		...
		VtkLassoStencilSourceWrap* obj = new VtkLassoStencilSourceWrap(native);
		obj->Wrap(info.This());
	}
	else
	{
...
void VtkLassoStencilSourceWrap::GetMTime(const Nan::FunctionCallbackInfo<v8::Value>& info)
{
	VtkLassoStencilSourceWrap *wrapper = ObjectWrap::Unwrap<VtkLassoStencilSourceWrap>(info.Holder());
	vtkLassoStencilSource *native = (vtkLassoStencilSource *)wrapper->native.GetPointer();
	unsigned int r;
