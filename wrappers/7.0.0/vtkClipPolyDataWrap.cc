...
#ifdef VTK_NODE_PLUS_VTKCLIPPOLYDATAWRAP_INITPTPL
	VTK_NODE_PLUS_VTKCLIPPOLYDATAWRAP_INITPTPL
#endif
	ptpl.Reset( tpl );
}
...
void VtkClipPolyDataWrap::New(const Nan::FunctionCallbackInfo<v8::Value>& info)
{
	...
	if(info.Length() == 0)
	{
		...
		VtkClipPolyDataWrap* obj = new VtkClipPolyDataWrap(native);
		obj->Wrap(info.This());
	}
	else
	{
...
void VtkClipPolyDataWrap::GetMTime(const Nan::FunctionCallbackInfo<v8::Value>& info)
{
	VtkClipPolyDataWrap *wrapper = ObjectWrap::Unwrap<VtkClipPolyDataWrap>(info.Holder());
	vtkClipPolyData *native = (vtkClipPolyData *)wrapper->native.GetPointer();
	unsigned int r;
