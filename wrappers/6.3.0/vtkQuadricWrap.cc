...
#ifdef VTK_NODE_PLUS_VTKQUADRICWRAP_INITPTPL
	VTK_NODE_PLUS_VTKQUADRICWRAP_INITPTPL
#endif
	ptpl.Reset( tpl );
}
...
void VtkQuadricWrap::New(const Nan::FunctionCallbackInfo<v8::Value>& info)
{
	...
	if(info.Length() == 0)
	{
		...
		VtkQuadricWrap* obj = new VtkQuadricWrap(native);
		obj->Wrap(info.This());
	}
	else
	{
...
void VtkQuadricWrap::EvaluateGradient(const Nan::FunctionCallbackInfo<v8::Value>& info)
{
	VtkQuadricWrap *wrapper = ObjectWrap::Unwrap<VtkQuadricWrap>(info.Holder());
	vtkQuadric *native = (vtkQuadric *)wrapper->native.GetPointer();
	size_t i;
