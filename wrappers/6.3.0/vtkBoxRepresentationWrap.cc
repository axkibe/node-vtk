...
#ifdef VTK_NODE_PLUS_VTKBOXREPRESENTATIONWRAP_INITPTPL
	VTK_NODE_PLUS_VTKBOXREPRESENTATIONWRAP_INITPTPL
#endif
	ptpl.Reset( tpl );
}
...
void VtkBoxRepresentationWrap::New(const Nan::FunctionCallbackInfo<v8::Value>& info)
{
	...
	if(info.Length() == 0)
	{
		...
		VtkBoxRepresentationWrap* obj = new VtkBoxRepresentationWrap(native);
		obj->Wrap(info.This());
	}
	else
	{
...
void VtkBoxRepresentationWrap::GetBounds(const Nan::FunctionCallbackInfo<v8::Value>& info)
{
	VtkBoxRepresentationWrap *wrapper = ObjectWrap::Unwrap<VtkBoxRepresentationWrap>(info.Holder());
	vtkBoxRepresentation *native = (vtkBoxRepresentation *)wrapper->native.GetPointer();
	double const * r;
