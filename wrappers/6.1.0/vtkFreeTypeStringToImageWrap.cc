...
#ifdef VTK_NODE_PLUS_VTKFREETYPESTRINGTOIMAGEWRAP_INITPTPL
	VTK_NODE_PLUS_VTKFREETYPESTRINGTOIMAGEWRAP_INITPTPL
#endif
	ptpl.Reset( tpl );
}
...
void VtkFreeTypeStringToImageWrap::New(const Nan::FunctionCallbackInfo<v8::Value>& info)
{
	...
	if(info.Length() == 0)
	{
		...
		VtkFreeTypeStringToImageWrap* obj = new VtkFreeTypeStringToImageWrap(native);
		obj->Wrap(info.This());
	}
	else
	{
...
void VtkFreeTypeStringToImageWrap::SetScaleToPowerOfTwo(const Nan::FunctionCallbackInfo<v8::Value>& info)
{
	VtkFreeTypeStringToImageWrap *wrapper = ObjectWrap::Unwrap<VtkFreeTypeStringToImageWrap>(info.Holder());
	vtkFreeTypeStringToImage *native = (vtkFreeTypeStringToImage *)wrapper->native.GetPointer();
