/* this file has been autogenerated by vtkNodeJsWrap */
/* editing this might proof futile */

#ifndef NATIVE_EXTENSION_VTK_VTKSHRINKFILTERWRAP_H
#define NATIVE_EXTENSION_VTK_VTKSHRINKFILTERWRAP_H

#include <nan.h>

#include <vtkSmartPointer.h>
#include <vtkShrinkFilter.h>

#include "vtkUnstructuredGridAlgorithmWrap.h"
#include "../../plus/plus.h"

class VtkShrinkFilterWrap : public VtkUnstructuredGridAlgorithmWrap
{
	public:
		using Nan::ObjectWrap::Wrap;
		static void Init(v8::Local<v8::Object> exports);
		static void InitPtpl();
		static void ConstructorGetter(
			v8::Local<v8::String> property,
			const Nan::PropertyCallbackInfo<v8::Value>& info);

		VtkShrinkFilterWrap(vtkSmartPointer<vtkShrinkFilter>);
		VtkShrinkFilterWrap();
		~VtkShrinkFilterWrap( );
		static Nan::Persistent<v8::FunctionTemplate> ptpl;

	private:
		static void New(const Nan::FunctionCallbackInfo<v8::Value>& info);

		static void GetClassName(const Nan::FunctionCallbackInfo<v8::Value>& info);
		static void GetShrinkFactor(const Nan::FunctionCallbackInfo<v8::Value>& info);
		static void GetShrinkFactorMaxValue(const Nan::FunctionCallbackInfo<v8::Value>& info);
		static void GetShrinkFactorMinValue(const Nan::FunctionCallbackInfo<v8::Value>& info);
		static void IsA(const Nan::FunctionCallbackInfo<v8::Value>& info);
		static void NewInstance(const Nan::FunctionCallbackInfo<v8::Value>& info);
		static void SafeDownCast(const Nan::FunctionCallbackInfo<v8::Value>& info);
		static void SetShrinkFactor(const Nan::FunctionCallbackInfo<v8::Value>& info);

#ifdef VTK_NODE_PLUS_VTKSHRINKFILTERWRAP_CLASSDEF
		VTK_NODE_PLUS_VTKSHRINKFILTERWRAP_CLASSDEF
#endif
};

#endif
