...
#ifdef VTK_NODE_PLUS_VTKGEOMETRYFILTERWRAP_INITPTPL
	VTK_NODE_PLUS_VTKGEOMETRYFILTERWRAP_INITPTPL
#endif
	ptpl.Reset( tpl );
}
...
void VtkGeometryFilterWrap::GetOutputPointsPrecision(const Nan::FunctionCallbackInfo<v8::Value>& info)
{
	VtkGeometryFilterWrap *wrapper = ObjectWrap::Unwrap<VtkGeometryFilterWrap>(info.Holder());
	vtkGeometryFilter *native = (vtkGeometryFilter *)wrapper->native.GetPointer();
	int r;
...
void VtkGeometryFilterWrap::SetOutputPointsPrecision(const Nan::FunctionCallbackInfo<v8::Value>& info)
{
	VtkGeometryFilterWrap *wrapper = ObjectWrap::Unwrap<VtkGeometryFilterWrap>(info.Holder());
	vtkGeometryFilter *native = (vtkGeometryFilter *)wrapper->native.GetPointer();
	if(info.Length() > 0 && info[0]->IsInt32())
